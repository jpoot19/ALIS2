...
function playSugerenciasNext(strVal, callback) {
    // console.log(jQuery.param({q:strVal}));
    $.get('/dictado/findNextWordDB?' + jQuery.param({ word: strVal }), callback, 'json');

}

function sugerencias_rapidas(strVal) {
    var html = "";
    $('#sugerencias').html('');
    $.get('/dictado/getsuggests', { q: strVal }, function(data) {
        $.each(data, function(key, value) {
            if (data[key] == 1) html += '<div class="cuadro2">' + key + '</div>';
            else html += '<div class="cuadro">' + key + '</div>';
        });
        $('#sugerencias').append(html);
    });
}

function sugerencias_rapidas_siguiente_palabra(strVal) {
    var html = "";
    $('#sugerencias').html('');
    $.get('/dictado/findNextWordDB', { word: strVal }, function(data) {
        $.each(data, function(key, value) {
            console.log(value);
            html += '<div class="cuadro3">' + value + '</div>';
        });
        $('#sugerencias').append(html);
    });
}
...
function loadFacebookContacts() {
    var contactsModal = $("#selectContactModal");
    var contactsContainer = $("#selectContactModal .modal-body");
    contactsContainer.html('');
    $.get('/configuracion/getFacebookContacts', function(contacts) {
        var auxBtn;
        for (var c in contacts) {
            auxBtn = $("<button/>");
            auxBtn.attr('type', 'button');
            auxBtn.attr('onclick', 'sendMessageFacebook(this)');
            auxBtn.data('audio', contacts[c].name);
            auxBtn.attr('class', "btn btn-default menuOption");
            auxBtn.data('chat', contacts[c].idChat);
            auxBtn.html("<img src='/images/person.png'><span>" + contacts[c].name + "</span>");
...
function sendMessageFacebook(btn) {
    var btn = $(btn);
    var idChat = btn.data('chat');
    var toPerson = btn.data('audio');
    monitorModal.find(".monitorTitle._1").text("Mensaje:");
    ...
    if (currentMsg) {
        playTextToSpeech("enviando");
        $(document.body).modalmanager('loading');
        $.get('/dictado/sendMessageToContactFacebook', { idChat: idChat, msgToSend: currentMsg }, function(r) {
            if (r.success) {
                playTextToSpeech("Mensaje, " + currentMsg + ". Enviado a " + toPerson);
                monitorModal.modal('show');
            } else {
...
function searchOnYoutube() {
    var keyWord = $('#boxMsg').text();
    window.location.href = "/videosYT?keyWord=" + keyWord;
